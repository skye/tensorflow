...
// Function that takes cond subgraph inputs and return cond subgraph boolean
// output. 'output' need not be set if an error is returned.
typedef std::function<Status(const Scope&, const std::vector<Output>& inputs,
                             Output* output)>
    CondGraphBuilderFn;

// Function that takes body subgraph inputs and return body subgraph
// outputs. 'outputs' need not be populated if an error is returned.
typedef std::function<Status(const Scope&, const std::vector<Output>& inputs,
...
// * cond: a function that builds the condition subgraph of the loop. Takes the
//     current loop variables as inputs and returns a boolean output indicating
//     whether the loop should continue.
// * body: a function that builds the body subgraph of the loop. Takes the
//     current loop variables as inputs and returns the updated loop variables.
// * frame_name: the frame name to use for this while loop
...
Status BuildWhileLoop(const Scope& scope, const std::vector<Output>& inputs,
                      ...
                      OutputList* outputs, Output* cond_output = nullptr);
