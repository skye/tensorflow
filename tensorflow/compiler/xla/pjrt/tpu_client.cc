...
ABSL_FLAG(absl::Duration, jax_tpu_init_retry_timeout, absl::Seconds(0),
          "Timeout for waiting for TPU devices to appear. NOTE: This value "
          "represents the amount of time after which JAX will stop asking "
          "the TPU runtime to initialize. Calls to initialize may take 300s "
          "or more so this time represents a lower bound.");
...
Status TpuDeviceState::ThenMemcpyDeviceToDevice(
  ...
  auto* transfer_tpu_stream =
      down_cast<tf_tpu::TpuStream*>(transfer_stream->implementation());
  tf_tpu::TpuTopologyExternal topology =
      tf_tpu::TpuPlatformInterface::GetRegisteredPlatform()->topology();
  if (topology.version() == kTpuV4) {
    auto* dst_tpu_stream =
        down_cast<tf_tpu::TpuStream*>(dst_stream->implementation());
...
StatusOr<absl::optional<std::string>> PjRtTpuClient::ExecutableFingerprint(
  ...
  xla::TpuExecutableInterface* tpu_executable =
      down_cast<xla::TpuExecutableInterface*>(
          executable.executables()[0]->executable());
  return absl::optional<std::string>(tpu_executable->fingerprint());
}

static StatusOr<std::vector<std::unique_ptr<PjRtDevice>>> GetTpuDevices(
    LocalClient* client,
  ...
  for (int i = 0; i < client->device_count(); ++i) {
    ...
    tf_tpu::TpuExecutorInterface* tpu_executor =
        down_cast<tf_tpu::TpuExecutorInterface*>(executor->implementation());
...
Status GetTpuClientArgs(bool asynchronous, LocalClient** local_client,
  ...
  auto start = absl::Now();
  auto retry_timeout = absl::GetFlag(FLAGS_jax_tpu_init_retry_timeout);
  ...
  while (!platform->Initialized()) {
    ...
    if (!status.ok()) {
      ...
      if ((absl::Now() - start) >= retry_timeout) {
        return status;
      }
    }
  }
  ...
  *local_client = client;
  *host_id = platform->GetTpuHostLocation().Id();
...
StatusOr<std::shared_ptr<PjRtClient>> GetTpuClient(bool asynchronous) {
  ...
  int host_id;
  tensorflow::tpu::TpuPlatformInterface* tpu_platform;
  TF_RETURN_IF_ERROR(GetTpuClientArgs(asynchronous, &client, &devices, &host_id,
                                      &tpu_platform));
  return std::shared_ptr<PjRtClient>(absl::make_unique<PjRtTpuClient>(
      client, std::move(devices), host_id, tpu_platform));
}
